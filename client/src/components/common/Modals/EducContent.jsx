...
function EducContent({ onClose, contentId }) {
  ...
  if (loading) {
    return (
      <div className="h-[75vh] flex flex-col items-center justify-center gap-3 text-gray-500 dark:text-neutral-400 transition-colors duration-300 ease-in-out">
  ...
  if (!contentDetails) {
    return (
      <div className="h-[75vh] flex flex-col items-center justify-center gap-3 text-gray-600 dark:text-neutral-400 transition-colors duration-300 ease-in-out">
        <i className="fas fa-circle-exclamation text-4xl text-yellow-500" />
        <div className="text-center max-w-sm">
          <h3 className="text-lg font-semibold mb-1 dark:text-neutral-50 transition-colors duration-300 ease-in-out">
  ...
  const renderMedia = () => {
    ...
    return mediaUrls.map((url, idx) => {
      ...
      return (
        <p
          key={idx}
          className="text-gray-500 dark:text-neutral-400 transition-colors duration-300 ease-in-out"
        >
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto max-h-[75vh] overflow-auto">
        ...
        <div className="bg-gray-50 dark:bg-neutral-900 rounded-xl p-4 shadow-md overflow-y-auto h-full flex flex-col justify-between transition-colors duration-300 ease-in-out">
          <div>
            <h2 className="text-3xl font-bold mb-2">
              <span className="bg-gradient-to-r from-teal-400 to-sky-600 bg-clip-text text-transparent">
                {title}
              </span>
            </h2>
            <p className="text-gray-700 dark:text-neutral-300 mb-4 transition-colors duration-300 ease-in-out">
              {description}
            </p>

            {relatedSymptoms.length > 0 && (
              <div className="mb-4">
                <h3 className="font-semibold text-gray-800 dark:text-neutral-50 transition-colors duration-300 ease-in-out">
                  Síntomas relacionados:
                </h3>
                <ul className="list-disc list-inside text-gray-600 dark:text-neutral-400 transition-colors duration-300 ease-in-out">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {clinicalTags.map((tag, idx) => {
                    ...
                    return (
                      <span
                        key={idx}
                        className={`flex items-center gap-1.5 px-3 py-1 text-xs rounded-full font-medium shadow-sm transition-colors duration-300 ease-in-out ${style}`}
                      >
            ...
            <div className="text-gray-500 dark:text-neutral-400 text-sm transition-colors duration-300 ease-in-out">
          ...
          {user?.role === "doctor" && (
            <div className="mt-4 flex gap-2">
              <Button
                onClick={() => setActiveEdit("editarContenido")}
                label="Editar"
                type="bg1"
                icon={"fa-pen"}
                full={true}
                classes={"!text-base !py-2"}
              />
              <Button
          ...
          {showConfirm && (
            <Modal
              type="alert"
              title="¿Estás seguro de eliminar este contenido?"
