...
function HistoryAppointment({ selectedAppointment }) {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6 transition-colors duration-300 ease-in-out">
      ...
      {errors.length > 0 && (
        <div className="bg-red-50 dark:bg-neutral-800 border border-red-300 dark:border-neutral-700 text-red-700 dark:text-red-400 p-4 rounded-xl shadow-sm space-y-1 transition-colors duration-300 ease-in-out">
          {errors.map((err, i) => (
            <p
              key={i}
